...
A global bilingual FAQ assistant remains closed by default and answers suggested or typed questions through deterministic client-side keyword scoring. Its English and Bulgarian FAQ records cover shipping, delivery timing, returns, custom work, payments, and optional accounts. Unknown questions receive related topics and a workshop email action. Conversation text remains only in React memory, resets on language change or refresh, and is never sent to the server, analytics, storage, or a third party.
