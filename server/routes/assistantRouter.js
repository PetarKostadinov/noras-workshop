...
export const validateAssistantRequest = ({ message, language } = {}) => {
  ...
  if (cleanMessage.length > MAX_MESSAGE_LENGTH) {
    throw Object.assign(new Error(`Questions must be ${MAX_MESSAGE_LENGTH} characters or fewer`), { status: 400 });
  }
  if (!SUPPORTED_LANGUAGES.has(language)) {
    throw Object.assign(new Error('Unsupported assistant language'), { status: 400 });
  }
  return { message: cleanMessage, language };
};

assistantRouter.post('/message', asyncHandler(async (req, res) => {
